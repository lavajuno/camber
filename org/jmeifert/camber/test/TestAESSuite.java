package org.jmeifert.camber.test;

import org.jmeifert.camber.security.AESSuite;

public class TestAESSuite {
    public static void main(String[] args) {
        System.out.println("Testing AESSuite...");
        System.out.println("Testing instantiation...");
        AESSuite a = new AESSuite();
        AESSuite b = new AESSuite(a.getKey());
        System.out.println("Testing encryption...");

        byte[] ct = new byte[0];
        try {
            ct = b.encryptString("The quick brown fox jumped over the lazy dog.");
        } catch(Exception e) {
            System.err.println(e.getMessage());
            return;
        }
        String dc = "";
        try {
            dc = a.decryptString(ct);
        } catch(Exception e) {
            System.err.println(e.getMessage());
            return;
        }

        ct = new byte[0];
        try {
            ct = b.encryptString("The");
        } catch(Exception e) {
            System.err.println(e.getMessage());
            return;
        }
        dc = "";
        try {
            dc = a.decryptString(ct);
        } catch(Exception e) {
            System.err.println(e.getMessage());
            return;
        }
        if(!dc.equals("The")) {
            System.err.println("Encryption and decryption test failed.");
            return;
        }
        System.out.println("Test of AESSuite passed.");
    }
}
